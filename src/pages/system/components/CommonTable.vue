        ...
        <t-col>
          <t-row :gutter="[16, 24]">
            ...
            <t-col>
              ...
              <t-button variant="base" :theme="buttonTheme" @click="toggleButton">
                {{ buttonText }}
              </t-button>
              <t-button variant="base" @click="moreFunctions" theme="warning"> 更多功能</t-button>
            </t-col>
          </t-row>
        </t-col>
      </t-row>
...
export default {
  ...
  methods: {
    toggleButton() {
      if (this.buttonTheme === 'danger') {
        this.buttonTheme = 'success';
        this.buttonText = '自动调控：开';
        this.updateValues(); // 初始化调用一次以确保立即开始更新数据
        this.timer = setInterval(() => {
          this.updateValues();
        }, 5000); // 每隔10秒调用一次updateValues函数
      } else {
        this.buttonTheme = 'danger';
        this.buttonText = '自动调控：关';
        clearInterval(this.timer); // 清除定时器
      }
    },
    updateValues() {
      const newValue = Math.floor(Math.random() * 31) + 70; // 生成70到100之间的随机数
      const newValue1 = Math.floor(Math.random() * 30) + 10; // 生成70到100之间的随机数
      this.sendMqttMessage('setHighBright', newValue);
      this.updateData(newValue + '%', 'high_bright');
      this.sendMqttMessage('setStandbyBright', newValue1);
      this.updateData(newValue1 + '%', 'standby_bright');
    },
    /**
     * 数据清除函数
     */
    ...
    sendMqttMessage(action, mode) {
      ...
      this.publish1("led/led", jsonString);
    },
    /**
     * 从订阅主题消息中获取相应键值数据
     */
