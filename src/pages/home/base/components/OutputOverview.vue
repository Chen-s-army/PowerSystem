<template>
  <t-card :bordered="false">
    <t-row>
      <t-col :xs="12" :xl="9">
        <t-card :bordered="false" title="传感器触发次数" subtitle="(次)"
          :class="{ 'dashboard-overview-card': true, 'overview-panel': true }">
          <template #actions>
            <t-date-range-picker class="card-date-picker-container" theme="primary" mode="date"
              :default-value="LAST_7_DAYS" @change="onStokeDataChange" />
          </template>
          <div id="stokeContainer" style="width: 100%; height: 351px" ref="stokeContainer"
            class="dashboard-chart-container"></div>
        </t-card>
      </t-col>
      <t-col :xs="12" :xl="3">
        <t-card :bordered="false" :class="{ 'dashboard-overview-card': true, 'export-panel': true }">
          ...
          <t-row>
            <t-col :xs="6" :xl="12">
              <t-card :bordered="false" subtitle="本月触发次数总计（次）" class="inner-card">
            ...
            <t-col :xs="6" :xl="12">
              <t-card :bordered="false" subtitle="上月触发次数总计（次）" class="inner-card">
                <div class="inner-card__content">
                  <div class="inner-card__content-title">2356</div>
                </div>
              </t-card>
            </t-col>
          </t-row>
